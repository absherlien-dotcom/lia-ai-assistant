...
صوتك وشخصيتك المفترضة: أنثوي عربي هادئ، العمر الصوتي 24 إلى 30 سنة، عربية بيضاء مفهومة مع لمسة يمنية خفيفة جدًا عند الحاجة.
لا تكوني موظفة خدمة عملاء، بل مساعدة شخصية راقية تفهم هشام.
عند التنبيهات: مختصرة وواضحة.
عند المحادثة: دافئة ومريحة.
عند المهام المهمة أو الديون: جدية أكثر بدون قسوة.
مهمتك تنظيم حياة هشام اليومية، أعماله، مهامه، ديونه، ملاحظاته، وقراراته.
إذا فهمتِ من الرسالة مهمة أو دين أو مصروف أو ملاحظة فأكدي له بلطف أن الأمر تم ترتيبه.
لا تخترعي معلومات، ولا تطلبي بيانات حساسة بلا ضرورة.
نادِ المستخدم باسم هشام عندما يكون مناسبًا.
`;

function parseCookies(cookieHeader = "") {
  return Object.fromEntries(cookieHeader.split(";").map(p=>p.trim()).filter(Boolean).map(p=>{
...
function setSessionCookie(res){
  ...
  res.setHeader("Set-Cookie", `lia_session=${makeToken()}; HttpOnly; SameSite=Lax; Path=/; Max-Age=31536000${secure}`);
}
...
function getGeminiKeyInfo(){
  for(const name of ["GEMINI_API_KEY","GEMINI_KEY","GOOGLE_API_KEY","GOOGLE_AI_API_KEY","GOOGLE_GENERATIVE_AI_API_KEY","GENERATIVE_LANGUAGE_API_KEY"]){
    const v=process.env[name];
    if(typeof v==="string" && v.trim()) return {key:v.trim(),source:name,length:v.trim().length};
  }
  ...
  for(const model of models()){
    try{
      const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${apiKey}`,{
        ...
        body:JSON.stringify({contents:[{role:"user",parts:[{text:prompt}]}],generationConfig:{temperature:.72,topP:.9,maxOutputTokens:1200}})
      });
      const data=safeJsonParse(await r.text());
      if(!r.ok){attempts.push({model,status:r.status,error:data?.error?.message||data?.error?.status||data?.raw});continue}
      return {ok:true,model,reply:data?.candidates?.[0]?.content?.parts?.[0]?.text||"يا هشام، وصلني الطلب لكن لم أستطع استخراج رد واضح الآن."};
    }catch(e){attempts.push({model,status:"FETCH_FAILED",error:e.message})}
  }
...
function route(req){
  ...
  if(!p || p==="api" || p==="index") p=url.searchParams.get("route")||"health";
  return p.replace(/^index\/?/,"").replace(/^\/+/,"");
}
export default async function handler(req,res){
  ...
  if(r==="google-login"){
    ...
    if(!GOOGLE_CLIENT_ID) {
      return res.status(500).json({ ok:false, error:"GOOGLE_CLIENT_ID غير مضاف في Vercel." });
    }
    ...
    try{
      ...
      if(accessToken){
        ...
        if(!tokenInfoRes.ok){
          return res.status(401).json({ ok:false, error:"تعذر التحقق من رمز Google." });
        }

        const audience = tokenInfo.aud || tokenInfo.audience;
        if(audience && audience !== GOOGLE_CLIENT_ID){
          return res.status(401).json({ ok:false, error:"Google Client ID غير مطابق لهذا الرمز." });
        }
        ...
        return res.status(400).json({ ok:false, error:"لم يصل رمز Google." });
      }

      if(LIA_ALLOWED_EMAIL && profile.email !== LIA_ALLOWED_EMAIL){
        return res.status(403).json({ ok:false, error:"هذا الحساب غير مصرح له بالدخول إلى ليا." });
      }
  ...
  if(r==="login"){
    ...
    if(username===LIA_USER && password===LIA_PASS){setSessionCookie(res);return res.json({ok:true})}
  ...
  if(r==="health"){
    ...
    return res.json({name:"LIA",version:"5.2",status:"online",brain:"Gemini",owner:"Hesham",hasGeminiKey:Boolean(k.key),geminiKeySource:k.source,geminiKeyLength:k.length,model:process.env.GEMINI_MODEL||"gemini-2.5-flash"});
  }
  if(r==="env-check"){
    if(!isLoggedIn(req)) return res.status(401).json({error:"UNAUTHORIZED"});
    const k=getGeminiKeyInfo();
    return res.json({hasGeminiKey:Boolean(k.key),geminiKeySource:k.source,geminiKeyLength:k.length,model:process.env.GEMINI_MODEL||"gemini-2.5-flash",envNames:Object.keys(process.env).filter(x=>/GEMINI|GOOGLE|API|KEY|LIA/i.test(x)).sort()});
  }
  if(r==="chat"){
    if(req.method!=="POST") return res.status(405).json({error:"METHOD_NOT_ALLOWED"});
    ...
    const k=getGeminiKeyInfo();
    if(!k.key) return res.json({reply:"يا هشام، مفتاح Gemini غير ظاهر داخل السيرفر. افتح /api/env-check للتأكد."});
...
أجيبي برد طبيعي فقط، بدون JSON. كوني عملية وذكية وقريبة.`;
    const result=await callGemini({apiKey:k.key,prompt});
    if(!result.ok){
      const e=result.attempts?.[0]||{};
      return res.json({reply:`يا هشام، Gemini رجّع خطأ:\nالموديل: ${e.model||"غير معروف"}\nالكود: ${e.status||"غير معروف"}\nالسبب: ${e.error||"غير معروف"}`});
    }
