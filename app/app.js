// LIA V5.4.3 — Google login + inline logo fallback
"use strict";
...
const LIA_LOGO_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><defs><linearGradient id="g" x1="90" y1="120" x2="420" y2="390"><stop offset="0" stop-color="#3CE8FF"/><stop offset=".52" stop-color="#7C4DFF"/><stop offset="1" stop-color="#EC5CFF"/></linearGradient><radialGradient id="bg" cx="50%" cy="42%" r="72%"><stop offset="0" stop-color="#18304D"/><stop offset=".55" stop-color="#0A102B"/><stop offset="1" stop-color="#050414"/></radialGradient><filter id="glow" x="-30%" y="-30%" width="160%" height="160%"><feGaussianBlur stdDeviation="10" result="b"/><feBlend in="SourceGraphic" in2="b" mode="screen"/></filter></defs><rect width="512" height="512" rx="122" fill="url(#bg)"/><circle cx="256" cy="255" r="160" fill="none" stroke="#3CE8FF" stroke-opacity=".18" stroke-width="2"/><circle cx="256" cy="255" r="118" fill="none" stroke="#EC5CFF" stroke-opacity=".22" stroke-width="2" stroke-dasharray="8 10"/><g filter="url(#glow)"><path d="M154 155C174 151 190 166 190 187V287C190 318 214 342 245 342H347C370 342 387 360 387 382C387 404 370 421 347 421H243C167 421 108 362 108 286V190C108 171 124 158 154 155Z" fill="url(#g)"/><path d="M200 342C245 313 265 275 285 238C316 178 371 181 407 227C359 207 330 230 303 282C274 338 236 363 200 342Z" fill="#fff" fill-opacity=".92"/><circle cx="395" cy="132" r="23" fill="#9B5CFF"/></g><text x="256" y="465" text-anchor="middle" font-family="Arial,sans-serif" font-size="54" font-weight="900" fill="#fff">lia.ai</text></svg>`;
const LIA_LOGO_DATA = "data:image/svg+xml;charset=utf-8," + encodeURIComponent(LIA_LOGO_SVG);
...
const titles = { chat: "كيف أساعدك اليوم؟", voice: "محادثة صوتية", tasks: "المهام", finance: "الديون والمصاريف", memory: "ذاكرة ليا", settings: "إعدادات ليا" };
...
function normalizeLogos(root=document){
  root.querySelectorAll("img").forEach(img=>{
    const src = img.getAttribute("src") || "";
    const alt = (img.getAttribute("alt") || "").toLowerCase();
    if(src.includes("lia-icon") || src.includes("lia-mark") || alt.includes("lia") || alt === "l"){
      img.src = LIA_LOGO_DATA;
...
function injectLoginStyles(){
  ...
  style.textContent = `
    ...
    .lia-logo-halo{width:132px;height:132px;margin:4px auto 12px;border-radius:42px;display:grid;place-items:center;position:relative;background:linear-gradient(145deg,rgba(60,232,255,.12),rgba(124,77,255,.12),rgba(236,92,255,.10));border:1px solid rgba(255,255,255,.13);box-shadow:0 0 42px rgba(60,232,255,.12),inset 0 0 36px rgba(255,255,255,.04)}.lia-logo-halo:before{content:"";position:absolute;inset:12px;border-radius:34px;border:1px solid rgba(60,232,255,.25)}.lia-logo-halo img{width:92px!important;height:92px!important;object-fit:contain!important;position:relative;z-index:2;filter:drop-shadow(0 0 22px rgba(60,232,255,.44))}.lia-brand-word{direction:ltr;font-weight:800;font-size:34px;letter-spacing:-.5px;margin-bottom:6px}.lia-brand-word span,.lia-grad{background:linear-gradient(90deg,#3ce8ff,#fff,#ec5cff);-webkit-background-clip:text;background-clip:text;color:transparent}
    ...
    @media(max-width:720px){.login-screen{padding:18px!important}.login-screen .login-card{width:100%!important}}
  `;
...
function prepareLoginUi(){
  ...
  if(card && !card.querySelector(".lia-login-inner")){
    ...
    inner.innerHTML = `<div class="lia-login-top"><span class="lia-pill">خاص بهشام</span><span class="lia-login-state" id="liaLoginStatus">فحص الدخول...</span></div><div class="lia-logo-halo"><img src="${LIA_LOGO_DATA}" alt="lia.ai"></div><div class="lia-brand-word">lia<span>.ai</span></div>`;
...
function showApp(){document.body.classList.add("lia-auth-app");if(loginScreen)loginScreen.classList.add("hide");if(appShell)appShell.style.display="grid";normalizeLogos(appShell||document);renderAll();restoreChat();checkHealth()}
...
function addMessage(role,text,skip=false){if(!messages)return null;const item=document.createElement("article");item.className=`message ${role==="user"?"user":"lia"}`;item.innerHTML=`<div class="avatar">${role==="user"?"هـ":`<img src="${LIA_LOGO_DATA}" alt="lia.ai"/>`}</div><div class="bubble"><span>${role==="user"?"هشام":"ليا"}</span><p>${esc(text)}</p></div>`;messages.appendChild(item);messages.scrollTop=messages.scrollHeight;if(!skip){state.chat.push({role:role==="user"?"user":"assistant",text});save()}if(role!=="user")lastReply=text;return item}
...
function notifyLocal(text){if(!("Notification" in window))return;if(Notification.permission==="granted")new Notification("ليا",{body:text,icon:LIA_LOGO_DATA});else if(Notification.permission!=="denied")Notification.requestPermission()}
...
prepareLoginUi();normalizeLogos(document);restoreChat();renderAll();checkHealth();loadVoices();authCheck();
