// LIA V5.4.5 — ElevenLabs TTS voice replies
"use strict";
...
const state = {
  ...
  settings: JSON.parse(localStorage.getItem("lia_v4_settings") || '{"tone":"yemeni-formal","voice":"","voiceLang":"ar-YE","autoSpeak":true}')
};
...
if(state.settings.autoSpeak !== true){state.settings.autoSpeak = true;save()}
...
function stopLiaAudio(){try{if(liaCurrentAudio){liaCurrentAudio.pause();liaCurrentAudio.currentTime=0;liaCurrentAudio.src="";liaCurrentAudio=null}}catch{}if("speechSynthesis" in window)speechSynthesis.cancel()}
function speakBrowserFallback(text){if(!("speechSynthesis" in window)){if(voiceStatus)voiceStatus.textContent="تعذر تشغيل الصوت.";return}const utter=new SpeechSynthesisUtterance(text);utter.lang=state.settings.voiceLang||"ar-YE";const voices=getVoices();const chosen=voices.find(v=>v.name===state.settings.voice)||bestArabicVoice()||voices[0];if(chosen)utter.voice=chosen;utter.rate=.94;utter.pitch=1.04;utter.onstart=()=>{if(voiceStatus)voiceStatus.textContent="ليا تتحدث الآن...";voiceOrb?.classList.add("speaking")};utter.onend=()=>{if(voiceStatus)voiceStatus.textContent="جاهزة لسماعك يا هشام.";voiceOrb?.classList.remove("speaking")};speechSynthesis.cancel();speechSynthesis.speak(utter)}
async function speak(text){const clean=String(text||"").replace(/[*#`_]/g,"").replace(/\s+/g," ").trim().slice(0,1200);if(!clean)return;stopLiaAudio();if(voiceStatus)voiceStatus.textContent="ليا تجهز صوتها...";voiceOrb?.classList.add("speaking");try{const r=await fetch("/api/tts",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:clean})});if(!r.ok)throw new Error(await r.text());const blob=await r.blob();const url=URL.createObjectURL(blob);liaCurrentAudio=new Audio(url);liaCurrentAudio.onplay=()=>{if(voiceStatus)voiceStatus.textContent="ليا تتحدث الآن...";voiceOrb?.classList.add("speaking")};liaCurrentAudio.onended=()=>{URL.revokeObjectURL(url);voiceOrb?.classList.remove("speaking");if(voiceStatus)voiceStatus.textContent="جاهزة لسماعك يا هشام."};liaCurrentAudio.onerror=()=>{voiceOrb?.classList.remove("speaking");speakBrowserFallback(clean)};await liaCurrentAudio.play()}catch(error){voiceOrb?.classList.remove("speaking");console.warn("ElevenLabs TTS failed, using browser fallback",error);speakBrowserFallback(clean)}}function speakIfWanted(text){if(state.settings.autoSpeak!==false)speak(text)}
